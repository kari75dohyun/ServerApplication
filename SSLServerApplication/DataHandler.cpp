...
DataHandler::DataHandler(boost::asio::io_context& io, std::shared_ptr<SessionManager> session_manager, int zone_count)
...
void DataHandler::dispatch(const std::shared_ptr<SSLSession>& session, const json& msg) {
    dispatcher_.dispatch(session, msg);
}

void DataHandler::add_session(int session_id, std::shared_ptr<SSLSession> session) {
    g_logger->info("[DEBUG][TCP] DataHandler address: {}", (void*)this);
...
void DataHandler::remove_session(int session_id) {
    ...
    // 2. 반환받은 세션을 세션 풀로 반환 (락 범위 밖에서 안전하게)
...
void DataHandler::broadcast(const std::string& msg, int sender_session_id, std::shared_ptr<SSLSession> /*session*/) {
    ...
    session_manager_->for_each_session([&](const std::shared_ptr<SSLSession>& sess) {
        if (!sess) return;
...
// 전체 세션 순회
void DataHandler::for_each_session(const std::function<void(const std::shared_ptr<SSLSession>&)> fn) {
...
void DataHandler::start_monitor_loop()
{
    ...
    monitor_timer_.async_wait([this](const boost::system::error_code& ec) {
        if (!ec) {
            // 1. 전체 카운트
            g_logger->info("[SERVER] Active sessions: {}", session_manager_->get_total_session_count());

            // 2. (추가) 세션 상세 정보 출력
            if (session_pool_) {
                size_t active_count = 0;
                session_pool_->for_each_active([&active_count](const std::shared_ptr<SSLSession>& sess) {
                    ++active_count;
                    g_logger->info("[ACTIVE] session_id={}, nickname={}", sess->get_session_id(), sess->get_nickname());
                    });
